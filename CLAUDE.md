...
**읽기 감시(`read_watch_paths`, §15) — 4·5차 (2026-09-22, 커널 -139).**
D 위에 읽기 감시를 얹는 티어 R · I · I2 를 쟀다. 읽기 지배 `w_find` 에서 D 대비:
cgroup 먼저(R) **+3.6%** · inode 먼저 + `BPF_CORE_READ`(I) **+2.0%** ·
inode 먼저 + **직접 load(I2) +0.57%** → D 를 합쳐도 최악 **≈ 0.93%**.
- **`BPF_CORE_READ` 는 포인터 한 단계마다 헬퍼를 부른다.** I 비용의 70%(58/82ns)가
  그것이었다. **제품 판정 함수는 BTF 포인터를 직접 따라간다.** 앞문 `f_mode` 도 같은
  방식으로 싸질 수 있으나 **미측정이다.**
- 4차의 "조회 순서는 무관"은 **철회** — 노이즈(±32ns)에 묻힌 판정이었고, 노이즈 1~3ns 인
  5차에서 R − I = 70ns. 같은 I 가 4차 147ns · 5차 82ns — **실행을 섞지 말 것.**
