...
## 파일 (2026-09-09 초안 → 2026-09-23 확정)
...
## 확정 검토에서 고친 것 (2026-09-23)

- **`policy_id` 는 불변 정책 버전 id 다.** `rule_*` 맵 키가 `(policy_id, …)` 인데 정책은 영장마다
  인라인으로 서명돼 내려온다. 발급 사이에 정책을 고치면 같은 `policy_id` 로 다른 규칙이 도착해
  노드 맵에서 서로 덮어쓴다. **정책을 수정할 때마다 서버가 새 u32 를 발급하고 재사용하지 않는다.**
  warrantd 는 같은 `policy_id` 에 내용이 다른 `Policy` 가 오면 적용하지 않고 경보한다.
  서버 `Policy` 엔티티는 UUID 하나라 u32 버전 컬럼이 필요하다.
- **`KillSwitch` 를 뺐다 (`PushMessage` 3 · `reserved`).** 영장은 Ed25519 로 서명하면서
  전 훅을 여는 메시지는 mTLS 만 믿고 받았다 — 서명을 둔 이유가 전송 경로를 믿지 않는 것이었다.
  이번 학기는 감사 모드라 열어 줄 차단이 없다. 강제 모드 때 서명 봉투로 다시 설계한다.
- **멱등 키를 `(node_id, seq_epoch, node_seq)` 로 바꿨다.** `node_id` 는 proto 에 없었고
  (`hostname` 은 노드 자기 신고), bbolt 가 날아가면 `node_seq` 가 1 로 돌아가서
  `ON CONFLICT DO NOTHING` 이 새 이벤트를 **중복으로 보고 조용히 버린다** — "빈 구간을
  숨기지 않는다"(§13 · §14)의 정면 위반. `node_id` 는 mTLS 인증서에서 뽑고,
  `AuditBatch.seq_epoch`(6) · `AuditAck.seq_epoch`(2)를 추가했다.
- **`WriteRule.Effect` 의 0 을 `EFFECT_UNSPECIFIED` 로.** 기존엔 `ALLOW = 0` 이라 `effect` 를
  빠뜨린 DENY 규칙이 허용 규칙이 됐다. 컴파일러가 UNSPECIFIED 를 거부한다. (아직 저장된
  데이터가 없어 번호를 옮겨도 깨지는 게 없다 — 확정 뒤로는 불가.)

## 미해결 — 전부 닫힘 (2026-09-23)
...
**한 사람 · 같은 호스트 · 같은 계정의 활성 영장은 최대 1개 — 서버 불변식이다 (2026-09-23).**
새 발급은 기존 영장을 대체한다: 서버가 기존 것의 취소 revision 을 **먼저** push 하고 새 영장을
push 한다(같은 스트림이라 순서가 보장된다). 모호함이 노드까지 내려가지 않으므로 warrantd 에
고르는 규칙이 필요 없다. `LookupResponse` 는 `repeated` 를 유지하되 0~1 개다 — 둘 이상이
오면 서버 버그로 보고 붙이지 않고 경보한다(로그인은 허용, §17).

`SSH_AUTH_INFO_0` 타이밍 미실측은 **proto 확정을 막지 않는다** — 어디서 읽든 필드는 같다.

### 2. `read_watch_paths` 를 실제로 넣을 것인가 — ✅ 넣는다, I2 방식 (2026-09-22)

**넣는다. 커널 구현은 "감시 목록 `(dev, ino)` 먼저 + BTF 포인터 직접 load".**
5 · 6차(`bench/overhead/out/20260922-234417` · `20260923-005559`) 읽기 지배 `w_find` 에서
D 대비 **+0.57~1.78%**, D 를 합쳐 최악 조건 **≈ 0.9~2.0%** — 기계 상태에 따라 흔들리지만
한 자릿수 % 는 넉넉히 통과한다. **R > I > I2 순위는 두 실행 모두 같다** — 결정의 근거는 이 순위다.

- cgroup 먼저(R)는 +3.6~4.9%, `BPF_CORE_READ` 로 읽으면(I) +2.0~3.6% — **둘 다 쓰지 않는다.**
- 비용은 무영장 세션의 읽기에도 똑같이 붙는다(목록을 cgroup 보다 먼저 본다).
- 필드 주석의 "미검증"은 이 결과로 해소됐다. "비어 있으면 BPF 가 읽기 경로를 보지
  않는다"는 규칙은 유지한다 — 목록이 비면 이 비용을 아예 안 낸다.
...
### 3. `WriteRule` 최장 일치를 커널에서 어떻게 구현하나 — ✅ proto 쪽은 닫음 (2026-09-23)

**의미만 proto 에 박는다: 깊이 상한을 넘으면 "안 걸림" = 금지.** 상한 값과 순회 방식(①조상 순회 ·
②하위 디렉터리 선등록, 필드 문서 §06)은 커널 담당이 정한다 — 어느 쪽이든 스키마는 같다.
순회를 넣으면 `bench/overhead` 로 다시 잰다.
...
- **`policy_id` 재사용 금지**: 정책 버전 하나에 id 하나. 고치면 새 id. 같은 id 에 다른 내용이 오면 warrantd 가 거부한다.
